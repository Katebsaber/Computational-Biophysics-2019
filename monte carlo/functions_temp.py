...
def move_one_particle(box,particles_pos_idx):
    ...
    # select move
    move_id = np.random.randint(0,4)

    # move right
    if move_id == 0:    
        right_id = particle_idx + right 
        if  box[right_id[1],right_id[0]] == 0: # if true the right cell is empty
            box[particle_idx[1],particle_idx[0]] = 0
            box[right_id[1],right_id[0]] = 1
            return True, box , get_particles_pos(box), move_id, particle_idx
        else:
            return False, box , get_particles_pos(box), move_id, particle_idx

    # move left
    if move_id == 1:    
        left_id = particle_idx + left
        if  box[left_id[1],left_id[0]] == 0: # if true the left cell is empty
            box[particle_idx[1],particle_idx[0]] = 0
            box[left_id[1],left_id[0]] = 1
            return True, box , get_particles_pos(box), move_id, particle_idx
        else:
            return False, box , get_particles_pos(box), move_id, particle_idx
    ...
    if move_id == 2:    
        ...
        if  box[upper_id[1],upper_id[0]] == 0: # if true the upper cell is empty
            box[particle_idx[1],particle_idx[0]] = 0
            box[upper_id[1],upper_id[0]] = 1
            return True, box , get_particles_pos(box), move_id, particle_idx
        else:
            return False, box , get_particles_pos(box), move_id, particle_idx
    ...
    if move_id == 3:    
        ...
        if  box[lower_id[1],lower_id[0]] == 0: # if true the lower cell is empty
            box[particle_idx[1],particle_idx[0]] = 0
            box[lower_id[1],lower_id[0]] = 1
            return True, box , get_particles_pos(box), move_id, particle_idx
        else:
            return False, box , get_particles_pos(box), move_id, particle_idx
